...
Instead of marking messages as read one-by-one, the `markMessageRead` method uses a high-watermark approach. When a client passes a reference `messageId`, the service stamps `readAt` on all unread messages sent by the _other_ participant that were created at or before that reference message's timestamp in a single bulk operation.
