...
The `verifyMetaWebhook` method enforces Meta's specific challenge-response protocol. When subscribing the webhook URL in the Meta Developer Portal, Meta sends a `GET` request with a challenge string. The service verifies the token and returns the _raw string_ challenge (not a JSON object) to complete the handshake.
