...
describe('TimezoneUtil', () => {
    ...
    describe('normalizeTimezone', () => {
        it('should return a valid timezone as-is', () => {
            expect(TimezoneUtil.normalizeTimezone('America/Chicago')).toBe('America/Chicago');
        });
        ...
        it('should handle case-insensitive matching', () => {
            expect(TimezoneUtil.normalizeTimezone('asia/kolkata')).toBe('Asia/Kolkata');
        });
    ...
    describe('convertToUTC', () => {
        it('should convert a date string from a given timezone to a UTC Date object (start of day)', () => {
            ...
            expect(TimezoneUtil.convertToUTC(dateString, timezone, 'start')).toEqual(expectedUTCDate);
        });
        ...
        it('should convert a date string from a given timezone to a UTC Date object (end of day)', () => {
            ...
            expect(TimezoneUtil.convertToUTC(dateString, timezone, 'end')).toEqual(expectedUTCDate);
        });
    });
    ...
    describe('formatInTimezone', () => {
        it('should format a UTC Date object into a string for a given timezone', () => {
            ...
            expect(TimezoneUtil.formatInTimezone(date, timezone, 'YYYY-MM-DD HH:mm')).toBe('2023-07-04 08:00');
        });
    });
    ...
    describe('getUTCDateRange', () => {
        it('should return the start and end of a date range in UTC', () => {
            ...
            const { startUTC, endUTC } = TimezoneUtil.getUTCDateRange(startDate, endDate, timezone);
    ...
    describe('getTimezoneAbbreviation', () => {
        it('should return the correct timezone abbreviation', () => {
            ...
            expect(TimezoneUtil.getTimezoneAbbreviation('America/New_York', winterDate)).toBe('EST');

            // Test during daylight saving time
            const summerDate = new Date('2023-07-15T12:00:00Z');
            expect(TimezoneUtil.getTimezoneAbbreviation('America/New_York', summerDate)).toBe('EDT');

            expect(TimezoneUtil.getTimezoneAbbreviation('Asia/Kolkata', new Date())).toBe('IST');
        });
    });

    describe('areTimezonesSame', () => {
        it('should return true for equivalent timezones', () => {
            expect(TimezoneUtil.areTimezonesSame('Asia/Kolkata', 'asia/kolkata')).toBe(true);
        });

        it('should return false for different timezones', () => {
            expect(TimezoneUtil.areTimezonesSame('America/New_York', 'Asia/Kolkata')).toBe(false);
        });
    });
});
