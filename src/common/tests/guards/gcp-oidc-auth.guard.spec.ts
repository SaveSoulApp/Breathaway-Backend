...
describe(GcpOidcAuthGuard.name, () => {
  ...
  describe('canActivate', () => {
    ...
    it('should throw UnauthorizedException when audience does not match configured GCP_OIDC_AUDIENCE', async () => {
      configService.get.mockReturnValue(mockAudience);

      const mockTicket = {
        getPayload: jest.fn().mockReturnValue({
          iss: 'https://accounts.google.com',
          aud: 'https://different-service.a.run.app',
          email: 'pubsub-invoker@breathaway-dev.iam.gserviceaccount.com',
          email_verified: true,
        }),
      } as unknown as LoginTicket;
    ...
    it('should throw UnauthorizedException when email_verified is false', async () => {
      configService.get.mockReturnValue(mockAudience);
    ...
    it('should throw UnauthorizedException (confused deputy defense) when token is from a different GCP project', async () => {
      ...
      // Token generated by an attacker from a different GCP project targeting our URL audience
      const mockTicket = {
        getPayload: jest.fn().mockReturnValue({
          iss: 'https://accounts.google.com',
          aud: mockAudience,
          email:
            'attacker-service-account@malicious-project.iam.gserviceaccount.com',
          email_verified: true,
        }),
      } as unknown as LoginTicket;
      ...
      const context = createMockExecutionContext({
        headers: {
          authorization: 'Bearer valid-token-from-foreign-project',
        },
      });
    ...
    it('should throw UnauthorizedException when caller is not in explicit GCP_OIDC_ALLOWED_EMAILS whitelist', async () => {
      configService.get.mockImplementation((key: string) => {
        if (key === 'GCP_OIDC_AUDIENCE') return mockAudience;
        if (key === 'GCP_OIDC_ALLOWED_EMAILS') {
          return 'pubsub-invoker@breathaway-dev.iam.gserviceaccount.com,scheduler-invoker@breathaway-dev.iam.gserviceaccount.com';
        }
        return undefined;
      });
      ...
      const mockTicket = {
        getPayload: jest.fn().mockReturnValue({
          iss: 'https://accounts.google.com',
          aud: mockAudience,
          email: 'unauthorized-sa@breathaway-dev.iam.gserviceaccount.com',
          email_verified: true,
        }),
      } as unknown as LoginTicket;
    ...
    it('should return true when token matches explicit GCP_OIDC_ALLOWED_EMAILS whitelist', async () => {
      configService.get.mockImplementation((key: string) => {
        if (key === 'GCP_OIDC_AUDIENCE') return mockAudience;
        if (key === 'GCP_OIDC_ALLOWED_EMAILS') {
          return 'pubsub-invoker@breathaway-dev.iam.gserviceaccount.com';
        }
        return undefined;
      });
