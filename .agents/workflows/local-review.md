---
description: Does a local review
---
...
2.  **Run the script**: Execute the local review script:
    ...
    ```
    _Note: If you get a permission denied error, run `chmod +x .agents/scripts/run-flash-lite-review.sh` first. This script requires `GEMINI_API_KEY` to be present in the `.env` file (or exported in the environment)._
