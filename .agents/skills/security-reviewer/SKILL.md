...
- Public routes must be marked deliberately with a `@Public()` decorator — never by the
  _absence_ of a guard. Absence-of-guard is unauditable; explicit `@Public()` is grep-able.
...
- Resource-level authorization (not just role-level) must be checked explicitly — a user with
  role `MEMBER` accessing `/orders/:id` must be checked against _ownership_ of that order, not
